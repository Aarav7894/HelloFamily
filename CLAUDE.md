...
- **Keep Expo package patch versions aligned and rebuild native projects after upgrades.** An older SDK 57.0.11 dev-client could not resolve `ExpoPushTokenManager`; upgrading `expo-notifications` to 57.0.13 and regenerating the native dependencies fixed it. `lib/notifications.ts` now uses the supported package-root import.
